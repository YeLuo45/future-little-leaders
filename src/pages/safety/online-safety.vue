...
import { SAFETY_CATEGORIES, CATEGORY_INFO, getSafetyKnowledge } from '@/services/safetyService.js'
...
// 获取路由参数
const category = ref(SAFETY_CATEGORIES.ONLINE)
...
// 分类信息
const categoryName = computed(() => CATEGORY_INFO[category.value]?.name || '安全教育')
const categoryIcon = computed(() => CATEGORY_INFO[category.value]?.icon || '📚')
const categoryColor = computed(() => CATEGORY_INFO[category.value]?.color || '#3498DB')
