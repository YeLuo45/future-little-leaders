	...
	<view class="page-container" :class="{ 'dark-mode': isDarkMode }">
		...
		<view class="content-area" :class="{ 'dark-mode': isDarkMode }">
			...
			<view class="tab-container" :class="{ 'dark-mode': isDarkMode }">
				<view class="tab-item" :class="{ 'active': activeTab === 'faq', 'dark-mode': isDarkMode }"
					@tap="activeTab = 'faq'">
					常见问题
				</view>
				<view class="tab-item" :class="{ 'active': activeTab === 'feedback', 'dark-mode': isDarkMode }"
			...
			<view v-if="activeTab === 'faq'" class="faq-container">
				<view v-for="(item, index) in faqList" :key="index" class="faq-item"
					...
					<view class="faq-question" @tap="toggleFaq(index)" :class="{ 'dark-mode': isDarkMode }">
						...
						<text class="arrow" :class="{ 'expanded': item.expanded }">{{ item.expanded ? '↑' : '↓'
						}}</text>
					</view>
			...
			<view v-if="activeTab === 'feedback'" class="feedback-container" :class="{ 'dark-mode': isDarkMode }">
				...
				<view class="form-item">
					...
					<textarea class="feedback-textarea" :class="{ 'dark-mode': isDarkMode }" v-model="feedbackContent"
						placeholder="请详细描述您遇到的问题或建议..."
						:placeholder-style="isDarkMode ? 'color: #666;' : 'color: #999;'"></textarea>
				</view>
				...
				<view class="form-item">
					...
					<input class="contact-input" :class="{ 'dark-mode': isDarkMode }" v-model="contactInfo"
	...
	export default {
		...
		methods: {
			...
			contactCustomerService() {
				uni.showModal({
					title: '联系客服',
					content: '客服邮箱：yuershiguangji45@outlook.com',
